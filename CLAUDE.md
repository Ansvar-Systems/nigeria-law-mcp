# Nigeria Law MCP Server — Developer Guide
...
Nigeria Law MCP server providing English-language Nigerian federal legislation search via Model Context Protocol. Strategy A deployment (Vercel, bundled SQLite DB).
...
1. `scripts/ingest.ts` — fetches from LFRN + NigeriaLII — JSON seed files in `data/seed/`
...
- **LFRN** (lfrn.gov.ng) — Federal Ministry of Justice, Federal Republic of Nigeria
- **NigeriaLII** (nigerialii.org) — African Legal Information Institute
- **License:** Government Public Data
- **Languages:** English (en) — official language of Nigeria
- **Coverage:** Federal Acts of the National Assembly, Constitution, and extant legislation
- **Identifier:** Act title + year + Cap number (e.g., "Nigeria Data Protection Act 2023")
- **Citation:** "Section N, [Act Title Year]" or "s N"

## Deployment

- Vercel Strategy A: DB bundled in `data/database.db`, included via `vercel.json` includeFiles
- npm package: `@ansvar/nigeria-law-mcp` with bin entry for stdio
