#!/usr/bin/env tsx
/**
 * Drift detection for Nigeria Law MCP.
 *
 * Checks if upstream LFRN/NigeriaLII content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Nigeria Law MCP — Drift Detection');
  console.log('==================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'Nigeria-Law-MCP/1.0 drift-detect' },
      });
