#!/usr/bin/env tsx
/**
 * Database builder for Nigeria Law MCP server.
 *
...
function extractEuReferences(text: string): ExtractedEUReference[] {
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      const referenceType: EUReferenceType = /\b(implement|align|transpos|equivalent|draw|model)\b/i.test(referenceContext) ? 'implements' : 'references';
...
function buildDatabase(): void {
  console.log('Building Nigeria Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Nigerian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'NG');
    insertMeta.run('source', 'lfrn.gov.ng + nigerialii.org');
  ...
  console.log(
    ...
    `${totalDefs} definitions, ${totalEuDocuments} EU/intl documents, ${totalEuReferences} EU/intl references`
  );
