#!/usr/bin/env tsx
/**
 * Nigerian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.lawnigeria.com';
const PORTAL_NAME = 'LawNigeria';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/nigerian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Nigerian Law MCP — Data Freshness Check');
