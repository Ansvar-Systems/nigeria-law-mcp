/**
 * Tool registry for Nigeria Law MCP Server.
...
import { getNigerianImplementations, type GetNigerianImplementationsInput } from './get-nigerian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including Laws of the Federation of Nigeria (LFRN) and NigeriaLII. ' +
    ...
    'Call this FIRST when you need to understand what Nigerian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Nigerian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
      ...
      'All results are in English (official language of Nigeria). ' +
      ...
      properties: {
        query: {
          ...
          description:
            ...
            '"personal data" for exact phrase, data AND protection for both terms, term* for prefix.',
        },
        document_id: {
          type: 'string',
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from a Nigerian statute. ' +
      ...
      'Supports Act title references (e.g., "Nigeria Data Protection Act 2023"), abbreviations (e.g., "NDPA", "CAMA"), and Cap numbers. ' +
    ...
    inputSchema: {
      ...
      properties: {
        document_id: {
          type: 'string',
          description:
            'Statute identifier: Act title (e.g., "Nigeria Data Protection Act 2023"), ' +
            'abbreviation (e.g., "NDPA", "CAMA", "NITDA Act"), ' +
            'Cap number (e.g., "Cap C20 LFN"), or internal document ID.',
        },
        section: {
          type: 'string',
          description: 'Section number (e.g., "1", "24", "37"). Omit to get all provisions.',
        },
        provision_ref: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'Validate a Nigerian legal citation against the database — zero-hallucination check. ' +
      ...
      'Supports formats: "Section 1, Nigeria Data Protection Act 2023", "s 1 NDPA 2023", "Cap C20 LFN".',
    inputSchema: {
      type: 'object',
      properties: {
        citation: {
          type: 'string',
          description: 'Citation string to validate. Examples: "Section 24, NDPA 2023", "s 1, Cybercrimes Act 2015".',
        },
      },
      required: ['citation'],
    },
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Nigerian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Nigeria?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Nigerian legal citation per standard conventions. ' +
      'Three formats: "full" (formal, e.g., "Section 1, Nigeria Data Protection Act 2023"), ' +
      '"short" (abbreviated, e.g., "s 1 NDPA 2023"), "pinpoint" (section reference only, e.g., "s 1").',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Nigerian statute or provision is currently in force, amended, repealed, or not yet in force. ' +
    ...
    inputSchema: {
      ...
      properties: {
        document_id: {
          type: 'string',
          description: 'Statute identifier (Act title, abbreviation, Cap number, or ID).',
        },
        provision_ref: {
          type: 'string',
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU/international legal basis that a Nigerian statute draws from or references. ' +
      'Nigeria is not an EU member but several Nigerian laws draw from EU regulations and international conventions ' +
      '(e.g., NDPA 2023 draws from GDPR, Cybercrimes Act 2015 draws from the Budapest Convention). ' +
      'Returns EU/international document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Nigerian statute identifier.' },
        include_articles: {
          type: 'boolean',
          description: 'Include specific EU/international article references (default: false).',
          default: false,
        },
      },
      required: ['document_id'],
  ...
  {
    name: 'get_nigerian_implementations',
    description:
      'Find all Nigerian statutes that draw from or implement a specific EU directive, regulation, or international convention. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Nigerian statutes. ' +
      'Note: Nigeria implements international standards through domestic legislation, not EU transposition.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives/regulations and international conventions that have Nigerian implementing legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_nigerian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents with Nigerian implementing legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU/international legal basis for a SPECIFIC provision within a Nigerian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU/international alignment status for a Nigerian statute or provision. ' +
      'Detects references to EU directives/regulations, missing alignment status, outdated references. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_nigerian_implementations':
          result = await getNigerianImplementations(db, args as unknown as GetNigerianImplementationsInput);
          break;
