/**
 * search_eu_implementations — Search EU directives/regulations that have Nigerian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_nigerian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as nigerian_statute_count
  ...
  if (input.has_nigerian_implementation) {
    sql += ' HAVING nigerian_statute_count > 0';
  }
