/**
 * Golden contract tests for Nigeria Law MCP.
 *
 * Tests tool outputs against the golden-tests.json fixture file.
 * Runs an in-memory MCP client/server pair against the real database.
 *
 * Nightly-only assertions (upstream_text_hash, citation_resolves) are
 * guarded with `it.skipIf(!isNightly)` so they don't block local dev.
 */
...
const dbPath =
  process.env['NIGERIA_LAW_DB_PATH'] ?? join(__dirname, '..', '..', 'data', 'database.db');
...
describe.skipIf(!dbExists)(`Contract tests: ${fixture.mcp_name}`, () => {
  beforeAll(async () => {
    ...
    const server = new Server(
      { name: 'nigeria-law-test', version: '0.0.0' },
      { capabilities: { tools: {} } },
    );
  ...
  it('fixture file is valid', () => {
    ...
    expect(fixture.mcp_name).toBe('Nigeria Law MCP');
